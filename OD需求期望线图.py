...
cd = gpd.read_file(r'C:\').to_crs('EPSG:3857')

# 连接表
'''
# 相同名的col还是会命名成两列，所以和每次merge重命名一样
origin = pd.merge(od, cd, left_on = 'origin', right_on = 'id').drop(['id'], axis = 1)
od_index = pd.merge(origin, cd, left_on = 'dest', right_on = 'id').drop(['id'], axis = 1).reset_index()
od_df = od_index.append(vc_index) 
'''
...
od_gpd = gpd.GeoDataFrame(point_line(od_dff))
od_gpd
